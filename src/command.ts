...
export abstract class BotModule {
  ...
  public async execute(cmd: Command): Promise<HandlerResponse> {
    const allHandlers = Object.keys(this.handlers);
    if (cmd.method === "help") {
      if (cmd.args[0] && this.handlers[cmd.args[0]] === undefined) {
        // help arg is non-existent
        return {
          message: `${this._name} has no functionality called \`${cmd.method}\`. Try \`${this.prefix}${this._name} help\``
        };
      }
      if (!cmd.args[0]) return { message: this.help() };
      return { message: this.help(cmd.args[0]) };
    }
